...
export async function generateEconomicReport() {
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `최근 24시간 이내의 [미국 경제 동향], [미국 연준(Fed) 금리 동향], [나스닥 기술주 흐름]에 대한 최신 경제 뉴스를 검색하고 분석하여 아래 목차에 맞춰 블로그 포스트를 한국어로 작성해줘.
...
출력은 Markdown 형식으로 제공해줘.`,
    ...
    return {
      content: response.text || "콘텐츠를 생성할 수 없습니다.",
